...
func streamVideoHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Set the HTTP header for the video content
	w.Header().Set("Content-Type", "video/mp4")

	// Use a buffer to improve streaming performance
	...
	for {
		...
		// Copy chunk content into the buffer and write to the response
		copy(buf, chunk.Content)
		_, err = w.Write(buf[:len(chunk.Content)])
		if err != nil {
