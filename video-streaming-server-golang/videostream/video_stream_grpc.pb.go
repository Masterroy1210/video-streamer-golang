...
type VideoStreamClient interface {
	StreamVideo(ctx context.Context, in *VideoRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[VideoResponse], error)
}
...
func (c *videoStreamClient) StreamVideo(ctx context.Context, in *VideoRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[VideoResponse], error) {
...
type VideoStream_StreamVideoClient = grpc.ServerStreamingClient[VideoResponse]
...
type VideoStreamServer interface {
	StreamVideo(*VideoRequest, grpc.ServerStreamingServer[VideoResponse]) error
...
func (UnimplementedVideoStreamServer) StreamVideo(*VideoRequest, grpc.ServerStreamingServer[VideoResponse]) error {
...
func _VideoStream_StreamVideo_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(VideoStreamServer).StreamVideo(m, &grpc.GenericServerStream[VideoRequest, VideoResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type VideoStream_StreamVideoServer = grpc.ServerStreamingServer[VideoResponse]
